...
- **単発タスク(1 セッションで完結)**: `/fable-task` — ミッションの重装備は省き、
  品質規律(委譲・検証・正直な報告)は省かない軽装版
- **長期タスク(セッションをまたぐ)**: `/fable-mission` 以下のミッションプロトコル
- 迷ったら「明日も続きをやるか?」で判断する。/fable-task から /fable-mission への
  **昇格はいつでもできる(降格はできない)**ので、迷ったら軽い方から始める
- 完了したら: ミッションは `/fable-retro` で振り返る。単発タスクでも成長シグナルは
  inbox に捕獲する

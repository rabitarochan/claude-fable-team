...
# 3. 導入済みの各プロジェクトで setup を再実行 —— リリースノートに規約(rules.md)の
...
5. セッションが切れた・翌日になった    新セッションで /fable-team:resume-mission
...
│   ├── setup/             プロジェクト導入(規約の正本 rules.md を同梱)
...
│   ├── checkpoint/        状態の凍結
│   ├── resume-mission/    新セッションでの完全復帰
│   ├── retro/             完了後の知見収穫
│   ├── grow/              チーム自身の成長(シグナル蒸留→資産更新)
│   └── debug-mission/ verify/ brief/ judge/   手筋スキル(各 playbook.md 同梱)
...
- 検証の手順: `claude plugin validate .` → ローカルインストール →
  実プロジェクトで `/fable-team:setup` から一連の動作を確認する

...
> これは思想の記録であり、日々の運用手順は `CLAUDE.md`、実行の型は `.claude/skills/fable-*` にある。
...
チームでは `.fable-team/growth/` と `/fable-grow` がこれを担う:
...
単発タスク(1 セッション完結)は `/fable-task` が同じ品質規律の軽装版で受ける。
...
(セッション終了・翌日・コンテキスト圧迫)
   │
/fable-resume ── 新セッションが state.md から完全復帰 → /fable-work へ戻る
   │
   ▼ 完了定義をすべて満たしたら
/fable-retro ── 知見を SkDD Skill / memory へ収穫
```
...
| 機能 | チームでの用途 |
|---|---|
| サブエージェント(`.claude/agents/`) | 本チームの 7 名。モデル・ツール・役割を固定した専門家 |
| Agent ツールの並列起動 | scout ファンアウト、独立タスクの同時実行 |
| `isolation: worktree` | 複数エージェントが同時にファイルを変更するときの衝突回避 |
| スキル(`.claude/skills/`) | ミッション・ライフサイクルの型(`/fable-*`)。手順を人ではなく型に覚えさせる |
| `/loop` | `/fable-work` の自動継続。長時間の自走(無人ループモードの規律に従う) |

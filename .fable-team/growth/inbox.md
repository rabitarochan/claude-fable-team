...
- [ ] 2026-07-05 | task(handoff-review-4) | 摩擦 | 敵対レビューを CHANGELOG エントリ作成前の変更セットに実施したため、「エントリ欠落」が must-fix として検出された(作成は元より次工程の予定 = 手順ノイズ)。フレームワーク資産のレビューは CHANGELOG エントリ草案まで含めた完成形の変更セットに対して実施すべきかもしれない。再発したら /fable-team:grow で判断。(2026-07-05 sweep 5 で完成形(CHANGELOG 草案込み)に対してレビューを実施 → エントリ欠落指摘は再発せず。手順有効の実証 1 回 — 蒸留候補として強まった)

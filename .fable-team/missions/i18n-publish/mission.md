...
- [ ] フレームワーク内部(agents/ 全 7、skills/ 全 28、.claude-plugin/ の description)に
      日本語文字が残っていない(`[ぁ-んァ-ヶ一-龯]` の grep が 0 件)
- [ ] rules.md に「ユーザーの言語で対話し、ミッション状態ファイルもユーザーの言語で書く」
      規則が入っている
- [ ] 逆翻訳レビュー(英→日に訳し戻して原文と意味を突き合わせ)で、未対応の意味ドリフト指摘が 0 件
- [ ] README.md(英)/ README.ja.md(日)、HANDOFF.md(英)/ HANDOFF.ja.md(日本語原本)が
      存在し、相互リンクされている
- [ ] LICENSE(MIT)が存在する
- [ ] `claude plugin validate .` が通る
- [ ] GitHub の公開リポジトリ `rabitarochan/claude-fable-team` に push され、

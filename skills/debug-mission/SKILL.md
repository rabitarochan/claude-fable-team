...
description: >
  ...
  work to debugger / builder. Also triggered by "/fable-team:debug-mission" or "debug by the playbook".
---

# /fable-team:debug-mission — Launch the Debugging Playbook
...
1. **Read the bundled `playbook.md`** (in this repository:
   `${CLAUDE_PLUGIN_ROOT}/skills/debug-mission/playbook.md`; it ships with the skill in plugin distribution too)
2. **When delegating** (the default; see the CLAUDE.md routing table): to debugger (cause unknown, or after 2 failed attempts)
   ...
   `${CLAUDE_PLUGIN_ROOT}/skills/debug-mission/playbook.md` in the brief's "references"

...
description: >
  ...
  「/fable-verify」「手筋どおり検証して」でも起動する。
---

# /fable-verify — 検証手筋の起動
...
1. **本スキル同梱の `playbook.md` を読む**(このリポジトリでは
   `.claude/skills/fable-verify/playbook.md`。プラグイン配布でもスキルと一緒に付いてくる)
2. **委譲する場合**(原則こちら): verifier へ。ブリーフに必ず含める —
   観測すべき完了条件(期待結果を先に書く)/ 変更種別(API・CLI・設定・migration・UI 等。
   プレイブックのレシピ選択に使わせる)/ 参照として `.claude/skills/fable-verify/playbook.md`

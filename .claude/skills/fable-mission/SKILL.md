...
name: fable-mission
description: >
  セッションをまたぐ長期タスク(ミッション)を開始する。「/fable-mission <ゴール>」
  「長期タスクとして始めて」「ミッション化して」と言われたとき、または /cc で受けた
  タスクが数時間で終わらない規模だと判明したときに使う。
  調査 → 計画 → 承認を経て .fable-team/missions/<slug>/ に状態ファイル一式を作る。
---

# /fable-mission — ミッション開始
...
2. **完了の定義(Definition of Done)を観測可能な条件として書き下す。**
   曖昧さが高い場合のみ、質問は **最大 1 つ** に絞って明確化する(/cc と同じ流儀)
...
この計画で進めてよいですか?(承認後は /fable-work で実行、/loop /fable-work で自動継続)
```
